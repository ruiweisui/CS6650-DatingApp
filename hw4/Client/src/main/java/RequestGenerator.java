...
public class RequestGenerator {
    ...
    private SwipeDetails generateBody(){
        ...
        int swiper = ThreadLocalRandom.current().nextInt(1,5000);
        int swipee = ThreadLocalRandom.current().nextInt(1,1000000);
        int length = ThreadLocalRandom.current().nextInt(1,256);
        String comment = RandomStringGenerator(length);
